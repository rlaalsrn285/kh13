...
public class MemberController222 {
	...
	private MemberDao memberDao;
	 //회원가입
	  ...
	  @PostMapping("/change")
	  public String change(@ModelAttribute MemberDto mmdto, HttpSession ss) {
		  String idid = (String) ss.getAttribute("loginId");
		  //세션에 있는아이디 추출해서 idid에 변환해서 넣기
		  mmdto.setMemberId(idid);
		  //mmdto에 추출한 아이디 설정
		  MemberDto ffdto = memberDao.selectOne(idid);
		  //ffdto에 세션아이디 db 조회후 넣음
		  boolean isvvv = mmdto.getMemberPw().equals(ffdto.getMemberPw());
		  //세션에있는 아이디넣은비번과 세션아이디 상세조회한아디비번과 같은지 (현재 이해불가)
		  if(isvvv) {
				memberDao.updateMember(mmdto);
